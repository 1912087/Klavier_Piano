                ...
                <section>
                    <div class="item_list">
                        <p class="QA">Q & A</p>
                            ...
                            <table>
	                            ...
	                            <tbody>
									<%
										...
										ArrayList<QA> list = qaDAO.getList(pageNumber);
										for(int i = 0; i < list.size(); i++){
									%>
									<tr>
										<td><%= list.get(i).getQID() %></td>
										<!-- 게시글 제목을 누르면 해당 글을 볼 수 있도록 링크를 걸어둔다 -->
										<td><a href="view.jsp?QID=<%= list.get(i).getQID() %>">
											<%= list.get(i).getUserID() %></a></td>
										<td><a href="view.jsp?QID=<%= list.get(i).getQID() %>">
											<%= list.get(i).getQTITLE().replaceAll(" ", "&nbsp;")
			                            		.replaceAll("<", "&lt;").replaceAll(">","&gt;").replaceAll("\n","<br>") %></a></td>
										<td><a href="view.jsp?QID=<%= list.get(i).getQID() %>">
											<%= list.get(i).getQDATE().substring(0, 11) + list.get(i).getQDATE().substring(11, 13) + ":"
											+ list.get(i).getQDATE().substring(14, 16) %></a></td>
										<td><a href="view.jsp?QID=<%= list.get(i).getQID() %>">
											<%= list.get(i).getQCLICK() %></a></td>
							...
							<%
								}if(qaDAO.nextPage(pageNumber + 1)){
							%>
								<button class="custom-btn btn-5" onclick="location.href='Q&A.jsp?pageNumber=<%= pageNumber + 1 %>'"><span>
								<a id="btn">다음</a>
								</span></button>
