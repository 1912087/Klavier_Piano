...
<body>
	<%
		...
		if(!userID.equals(qa.getUserID())){
			...
			if(request.getParameter("QTITLE") == null || request.getParameter("QCONTENT") == null
				|| request.getParameter("QTITLE").equals("") || request.getParameter("QCONTENT").equals("")){
				PrintWriter script = response.getWriter();
				script.println("<script>");
				...
				int result = QADAO.update(qID, request.getParameter("QTITLE"), request.getParameter("QCONTENT"));
				// 데이터베이스 오류인 경우
				if(result == -1){
					PrintWriter script = response.getWriter();
					script.println("<script>");
